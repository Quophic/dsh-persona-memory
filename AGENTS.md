...
- **WebUI 记忆管理页**（`lib/admin.js` + `client/client.js`，0.1.13）：Host 用 `webServer` 注册 `/api/persona-memory/*` 路由（status/checkModel/configSave/update/delete/standingAdd/standingUpdate/standingRemove/rebuildVector，**loopback-only 防护**，同 dsh-ssh 围栏）；Client 是**预构建 bundle**（`window.__ModuleLoader__.load` 格式，经 `dsh.client` + `exports["./client"]` 声明加载，**非动态插件无 harness.handle**，用同源 fetch 调路由）；页面注册到 `settings.section` slot（id=persona-memory，order=25，label=记忆管理）；配置写回 = 当前 profile 的 `cordis.patch.yml` 中 persona-memory 段（`adminProfile` 配置项指定 profile 名，默认 web）；**Client 代码修改后必须重新 `pnpm add` + 重启 web 才生效**（预构建产物按包分发）；client 只依赖 react + 浏览器 fetch，**不 import 任何 @deepseek-ai client 包运行时值**（peerDependencies 已声明 react、dsh-client-runtime、dsh-client-ui-slots、dsh-host-webserver 等）。
